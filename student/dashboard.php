...
function createMailer(): PHPMailer
{
    ...
    $mail->Port       = 587;
    $mail->Timeout    = 5;   // seconds (fail fast if blocked)
    ...
    return "
        <div style='font-family: Arial, sans-serif; padding: 25px; border: 2px dashed #330066;
            ...
            <h4 style='color: #666; text-align: center; margin-bottom: 20px;
                       ...
                       letter-spacing: 2px;'>Appointment Confirmation</h4>
            ...
            <p style='color: #555; margin: 8px 0;'>
                ...
                <strong style='color:#330066;'>successfully booked</strong>.
            </p>
            ...
            <p style='color: #555; font-size: 13px; margin-top: 20px;
                      text-align: center;'>
                Please arrive at the health unit reception counter <strong>10 minutes before</strong> your scheduled time.
            </p>
            <p style='color: #888; font-size: 12px; margin-top: 10px;
                      text-align: center; font-style: italic;'>
                If you need to cancel or reschedule, please do so at least 24 hours in advance through your student dashboard.
            </p>
        </div>
    ";
}
    ...
    return "
        <div style='font-family: Arial, sans-serif; padding: 25px; border: 2px dashed #cc0000;
            ...
            <h4 style='color: #666; text-align: center; margin-bottom: 20px;
                       text-transform: uppercase; font-size: 11px;
                       border-bottom: 1px solid #ddd; padding-bottom: 10px;
                       letter-spacing: 2px;'>Appointment Cancelled</h4>

            <p style='color: #333; margin: 8px 0;'>Dear <strong>{$studentname}</strong>,</p>
            <p style='color: #555; margin: 8px 0;'>
                This is to confirm that your appointment at the <strong>UiTM Health Unit</strong>
                has been <strong style='color:#cc0000;'>cancelled</strong> as requested.
            </p>
            ...
            <p style='color: #555; font-size: 13px; margin-top: 20px;
                      text-align: center;'>
                You may book a new appointment anytime through your student dashboard.
            </p>
            <p style='color: #888; font-size: 12px; margin-top: 10px;
                      text-align: center; font-style: italic;'>
                If you have any questions, please contact the health unit during office hours.
            </p>
        </div>
    ";
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['book_appointment'])) {
    ...
    if (!empty($appointment_time)) {
        ...
        // ── Validation 0: Student must not already have a future Scheduled appointment ──
        $existing_stmt = $conn->prepare(
            "SELECT id FROM appointments WHERE studentno = ? AND status = 'Scheduled' AND appointment_datetime > NOW() LIMIT 1"
        );
        $existing_stmt->bind_param("s", $studentno);
        $existing_stmt->execute();
        $existing_stmt->store_result();
        $has_scheduled = $existing_stmt->num_rows > 0;
        ...
        if ($has_scheduled) {
            $message       = "You already have a scheduled appointment. Please cancel it before booking a new one.";
            $message_class = "error";

        } elseif ((int)$check_dt->format('i') !== 0) {
        ...
        // ── Validation 4: Overlap check ──
            ...
            if ($stmt->execute()) {
                ...
                try {
                    ...
                    // First attempt failed — try fallback SMTP (port 465 / SMTPS)
                    try {
                        ...
                        $mail2->SMTPAuth   = true;
                        $mail2->Username   = 'aqilfaris20062@gmail.com';
                        $mail2->Password   = 'aufcuiagnyyutnme';
                        ...
                        $mail2->setFrom('aqilfaris20062@gmail.com', 'UiTM Health Unit');
                        $mail2->addAddress($studentemail, $studentname);
                        $mail2->isHTML(true);
                        $mail2->Subject = 'UiTM Health Unit – Appointment Confirmed ✅';
                        $mail2->Body    = $receipt_template;
                        $mail2->send();
                        $email_sent = true;
                    } catch (Exception $e2) {
                        // Both attempts failed — log the actual error
                        error_log("PHPMailer error (port 587): " . $e->getMessage());
                        error_log("PHPMailer error (port 465): " . $e2->getMessage());
                    }
                }
                ...
                if ($email_sent) {
                    ...
                    $message       = "Appointment saved! Note: Email could not be sent. Your receipt is shown below.";
                    $message_class = "success";
...
// ─────────────────────────────────────────────────────────────────────────────
// HANDLE CHECK-IN (10 min before appointment)
// ─────────────────────────────────────────────────────────────────────────────
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['check_in_action'])) {
    $appt_id = (int)($_POST['appt_id'] ?? 0);
    $status  = $_POST['check_in_action']; // 'Coming' or 'Not Coming'

    if ($appt_id > 0 && in_array($status, ['Coming', 'Not Coming'])) {
        // Verify ownership and that appointment is within 10-minute window
        $stmt = $conn->prepare(
            "SELECT id, appointment_datetime FROM appointments
             WHERE id = ? AND studentno = ? AND status = 'Scheduled'
             AND appointment_datetime BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 10 MINUTE)"
        );
        ...
        if ($stmt->num_rows > 0) {
            // Fetch appointment datetime for email
            $stmt->bind_param("is", $appt_id, $studentno);
            $stmt->execute();
            $appt_data = $stmt->get_result()->fetch_assoc();
            $formatted_time = date('F j, Y, g:i a', strtotime($appt_data['appointment_datetime']));

            // Update check-in status and mark as Completed if Coming, Cancelled if Not Coming
            if ($status === 'Coming') {
                $upd = $conn->prepare("UPDATE appointments SET check_in = ?, status = 'Completed' WHERE id = ?");
            } else {
                $upd = $conn->prepare("UPDATE appointments SET check_in = ?, status = 'Cancelled' WHERE id = ?");
            }
            $upd->bind_param("si", $status, $appt_id);
            if ($upd->execute()) {
                $label = $status === 'Coming' ? 'I\'m coming' : 'Not coming';
                $message       = "You indicated: \"$label\" for your appointment.";
                $message_class = "success";
                ...
                try {
                    ...
                    $mail->isHTML(true);
                    if ($status === 'Coming') {
                        $mail->Subject = 'UiTM Health Unit – Attendance Confirmed ✓';
                        $mail->Body = "
                            <div style='font-family: Arial, sans-serif; padding: 25px; border: 2px solid #34c759;
                                        border-radius: 12px; max-width: 500px; margin: 20px auto;
                                        background-color: #f6fff8; box-shadow: 0 4px 6px rgba(0,0,0,0.05);'>
                                <h2 style='color: #1a7a2e; text-align: center; margin-top: 0; font-size: 24px;'>UiTM Health Unit</h2>
                                <h4 style='color: #666; text-align: center; margin-bottom: 20px; text-transform: uppercase;
                                            font-size: 11px; border-bottom: 1px solid #ddd; padding-bottom: 10px;
                                            letter-spacing: 2px;'>Attendance Confirmed</h4>
                                <p style='color: #333; margin: 8px 0;'>Dear <strong>{$studentname}</strong>,</p>
                                <p style='color: #555; margin: 8px 0;'>
                                    You have confirmed your attendance for your upcoming appointment.
                                </p>
                                <p style='margin: 8px 0;'><strong>Student ID:</strong> <span style='color: #333;'>{$studentno}</span></p>
                                <p style='margin-bottom: 5px; font-weight: bold; color: #1a7a2e;'>Your Appointment Time:</p>
                                <div style='background-color: #1a7a2e; color: white; padding: 15px; text-align: center;
                        ...
                        $mail->Subject = 'UiTM Health Unit – Not Attending';
                        $mail->Body = "
                            <div style='font-family: Arial, sans-serif; padding: 25px; border: 2px solid #ff3b30;
                                        border-radius: 12px; max-width: 500px; margin: 20px auto;
                                        background-color: #fffafa; box-shadow: 0 4px 6px rgba(0,0,0,0.05);'>
                                <h2 style='color: #c0251b; text-align: center; margin-top: 0; font-size: 24px;'>UiTM Health Unit</h2>
                                <h4 style='color: #666; text-align: center; margin-bottom: 20px; text-transform: uppercase;
                                            font-size: 11px; border-bottom: 1px solid #ddd; padding-bottom: 10px;
                                            letter-spacing: 2px;'>Not Attending</h4>
                                <p style='color: #333; margin: 8px 0;'>Dear <strong>{$studentname}</strong>,</p>
                                <p style='color: #555; margin: 8px 0;'>
                                    You have indicated that you <strong style='color:#c0251b;'>cannot attend</strong> your scheduled appointment.
                                </p>
                                <p style='margin: 8px 0;'><strong>Student ID:</strong> <span style='color: #333;'>{$studentno}</span></p>
                                <p style='margin-bottom: 5px; font-weight: bold; color: #c0251b;'>Your Appointment Time:</p>
                                <div style='background-color: #c0251b; color: white; padding: 15px; text-align: center;
                                            border-radius: 6px; font-weight: bold; font-size: 18px; text-decoration: line-through;'>
                                    {$formatted_time}
                                </div>
                                <p style='color: #555; font-size: 13px; margin-top: 20px; text-align: center;'>
                                    You may book a new appointment anytime through your student dashboard.
                    ...
                    error_log("Check-in email error: " . $e->getMessage());
                }
            }
        } else {
            $message       = "Check-in is only available within 10 minutes before your appointment.";
            $message_class = "error";
        }
    }
}
            ...
            if ($upd->execute()) {
                ...
                try {
                    ...
                    try {
                        ...
                        $mail2->addAddress($studentemail, $studentname);
                        $mail2->isHTML(true);
                        $mail2->Subject = 'UiTM Health Unit – Appointment Cancelled';
                        $mail2->Body    = $cancel_template;
                        $mail2->send();
                        $email_sent = true;
                    } catch (Exception $e2) {
                        // Both attempts failed — log the actual error
                        error_log("PHPMailer error (cancellation, port 587): " . $e->getMessage());
                        error_log("PHPMailer error (cancellation, port 465): " . $e2->getMessage());
                    }
                }
                ...
                if ($email_sent) {
                    ...
                    $message       = "Your appointment has been cancelled. Note: the confirmation email could not be sent. Your cancellation receipt is shown below.";
                    $message_class = "success";
...
// ─────────────────────────────────────────────────────────────────────────────
// CHECK: does this student already have a future (Scheduled) booking?
// ─────────────────────────────────────────────────────────────────────────────
$active_stmt = $conn->prepare(
    "SELECT id FROM appointments WHERE studentno = ? AND status = 'Scheduled' AND appointment_datetime > NOW() LIMIT 1"
);
$active_stmt->bind_param("s", $studentno);
$active_stmt->execute();
$active_stmt->store_result();
$has_active_booking = $active_stmt->num_rows > 0;
...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ── CHECK-IN BUTTONS ── */
        .btn-coming {
            background: rgba(52,199,89,0.10); color: #1a7a2e;
        ...
        .btn-coming:hover  { background: rgba(52,199,89,0.20); }
        .btn-coming:active { transform: scale(0.96); }
        .btn-coming.active {
            background: #1a7a2e; color: #fff; cursor: default;
        }
        .btn-coming.active:hover { background: #1a7a2e; }
        ...
        .btn-not-coming:hover  { background: rgba(255,59,48,0.20); }
        .btn-not-coming:active { transform: scale(0.96); }
        .btn-not-coming.active {
            background: #c0251b; color: #fff; cursor: default;
        }
        .btn-not-coming.active:hover { background: #c0251b; }
        ...
        .check-in-coming { background: rgba(52,199,89,0.10); color: #1a7a2e; }
        .check-in-not-coming { background: rgba(255,59,48,0.08); color: #c0251b; }
        .check-in-pending { background: var(--fill-1); color: var(--label-tertiary); }
        .check-in-soon {
            display: inline-block;
            font-size: 11px; font-weight: 600; color: var(--orange);
            background: rgba(255,149,0,0.10); padding: 3px 8px;
...
<nav class="navbar">
    <div class="navbar-inner">
        ...
        <div class="nav-right">
            ...
            <a href="../auth/logout.php" class="btn-logout" id="logout-btn">Sign Out</a>
        </div>
    </div>
</nav>
        ...
        <div class="card">
            <div class="form-body">
                <p class="form-description">
                    ...
                    A confirmation receipt will be sent to your registered email. When your appointment is within 10 minutes, you can confirm your attendance.
                </p>
                ...
                <?php if ($has_active_booking): ?>
                    <div class="alert" style="background: rgba(255,149,0,0.10); color: #b86200; margin-bottom: 18px;">
                        <div class="alert-icon" style="background: var(--orange); color: #fff;">!</div>
                        <span>You already have a scheduled appointment. Please cancel it before booking a new one.</span>
                    </div>
                <?php endif; ?>

                <form action="../student/dashboard.php" method="POST" id="booking-form">
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php
                                        ...
                                        $appt_time      = strtotime($row['appointment_datetime']);
                                        $now            = time();
                                        $ten_min_window = ($row['status'] === 'Scheduled' && $appt_time > $now && $appt_time <= $now + 600);
                                        $check_in_val   = $row['check_in'] ?? '';
                                    ?>
                                    <?php if ($ten_min_window && !$check_in_val): ?>
                                        <div style="display:flex;gap:5px;flex-wrap:wrap;">
                                            <form method="POST" action="../student/dashboard.php" style="margin:0;">
                                                <input type="hidden" name="appt_id" value="<?= (int)$row['id'] ?>">
                                                <button type="submit" name="check_in_action" value="Coming" class="btn-coming">I'm coming ✓</button>
                                            </form>
                                            <form method="POST" action="../student/dashboard.php" style="margin:0;">
                                                <input type="hidden" name="appt_id" value="<?= (int)$row['id'] ?>">
                                                <button type="submit" name="check_in_action" value="Not Coming" class="btn-not-coming">Not coming ✕</button>
                                            </form>
                                        </div>
                                    <?php elseif ($check_in_val === 'Coming'): ?>
                                        <span class="check-in-label check-in-coming">✓ I'm coming</span>
                                    <?php elseif ($check_in_val === 'Not Coming'): ?>
                                        <span class="check-in-label check-in-not-coming">✕ Not coming</span>
                                    <?php elseif ($can_cancel): ?>
                                        <form action="../student/dashboard.php" method="POST" class="cancel-form"
